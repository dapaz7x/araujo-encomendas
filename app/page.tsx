...
export default function Home() {
  ...
  function saveOrder(event:FormEvent<HTMLFormElement>) {
    ...
    const saved=persistOrder(order); setOrders(current=>[saved,...current]); setNow(new Date().toLocaleString('pt-BR')); setFeedback('Encomenda salva neste computador.'); setTimeout(()=>setFeedback(''),3000);
  }
  ...
  return <main>
    ...
    <form onSubmit={saveOrder}>
      <section className="intro no-print"><div><p className="eyebrow">Gerenciador de encomendas</p><h1>Nova encomenda</h1><p>Preencha os dados abaixo. A ficha será preparada para impressão em bobina de 80 mm.</p></div><div className="status-dot"><span /> Impressora Elgin i8</div></section>
      ...
      </div><aside className="side-column"><div className="summary-card"><p className="eyebrow">Resumo</p><h3>{filledItems.length} {filledItems.length===1?'item':'itens'}</h3><div><span>Tipo</span><strong>{orderType}</strong></div><button className="save-button" type="submit">Salvar encomenda</button><button className="outline-button" type="button" onClick={printOrder}>Imprimir em 80 mm</button>{feedback&&<p className="feedback">✓ {feedback}</p>}</div><div className="tip"><strong>Antes de imprimir</strong><p>Selecione a Elgin i8 e confirme o tamanho de papel 80 mm nas opções da impressora.</p></div><div className="recent"><h3>Recentes neste computador</h3>{orders.slice(0,3).map(order=><div key={order.id}><strong>{order.customer}</strong><span>{formatDate(order.deliveryDate)}</span></div>)}{orders.length===0&&<p>Nenhuma encomenda salva.</p>}<button type="button" onClick={()=>setManagerOpen(true)}>Ver e gerenciar todas →</button></div></aside></div>
    ...
    {helpOpen&&<HelpPanel onClose={()=>setHelpOpen(false)} onTest={printCalibration}/>} 
...
const statusLabels:Record<OrderStatus,string>={pending:'Pendente',production:'Em produção',ready:'Pronto',delivered:'Entregue'};
...
function HelpPanel({onClose,onTest}:{onClose:()=>void,onTest:()=>void}) { return <div className="help-overlay no-print" role="dialog" aria-modal="true" aria-labelledby="help-title" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><section className="help-panel"><header><div><p className="eyebrow">Configuração da impressora</p><h2 id="help-title">Elgin i8 · Papel 80 mm</h2></div><button type="button" onClick={onClose} aria-label="Fechar ajuda">×</button></header><div className="printer-badge"><strong>Área útil preparada: 72 mm</strong><span>Aproveita a largura de impressão da bobina sem encostar nas bordas.</span></div><h3>Na janela de impressão</h3><ol><li>Em <b>Destino</b>, escolha a impressora <b>Elgin i8</b>.</li><li>Selecione o papel <b>80 mm</b> ou <b>80 × comprimento personalizado</b>.</li><li>Deixe a escala em <b>100%</b>.</li><li>Use <b>Margens: nenhuma</b> e desative <b>Cabeçalhos e rodapés</b>.</li><li>Imprima uma ficha de teste antes do primeiro pedido real.</li></ol><h3>Se algo sair errado</h3><div className="fix-grid"><div><b>Conteúdo cortado</b><span>Confira papel 80 mm. Se continuar, use escala 95%.</span></div><div><b>Papel sobrando à direita</b><span>Volte a escala para 100% e confirme orientação Retrato.</span></div><div><b>Sai uma folha muito longa</b><span>Escolha no driver um tamanho de cupom contínuo/recibo.</span></div><div><b>Letra fraca</b><span>Aumente a densidade de impressão nas preferências da Elgin.</span></div></div><div className="help-actions"><button type="button" className="outline-button" onClick={onClose}>Fechar</button><button type="button" className="save-button" onClick={onTest}>Imprimir teste de largura</button></div><p className="help-note">Dica: pressione F1 a qualquer momento para abrir este painel.</p></section></div>; }
